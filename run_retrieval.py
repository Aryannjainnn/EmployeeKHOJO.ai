...
def main() -> None:
    ...
    parser.add_argument("--out-prefix", default="results", help="Prefix for the 3 output files")
    parser.add_argument("--top-k",      type=int, default=20,  help="Number of results per file")
    ...
    file_map = {
        "hybrid":   f"{args.out_prefix}_hybrid.json",
        "lexical":  f"{args.out_prefix}_lexical.json",
        "semantic": f"{args.out_prefix}_semantic.json",
    }

    for mode, path in file_map.items():
        with open(path, "w", encoding="utf-8") as f:
            json.dump(output[mode], f, indent=2, ensure_ascii=False)
        print(f"[Written] {path}  ({output[mode]['meta']['returned']} results)")
